...
export class NewCustomer extends React.Component {
	...
	handleSubmit = (e) =>{
		...
		this.props.setNewCustomer(false)
	}
	render() {

		return (
			<div className={styles.main} onClick={(e)=>{this.props.setNewCustomer(false)}}>
			<form className={styles.form} onSubmit={this.handleSubmit} onClick={e=>e.stopPropagation()}>
				...
				<div className={styles.buttons}><button className={styles.submit} type="submit">Save</button><div className={styles.cancel} onClick={(e)=>{this.props.setNewCustomer(false)}}>Cancel</div></div>
			</form>
			</div>
		)
	}
